...
namespace Core.ByteEncryption
{
    ...
    class bytesGetter
    {
        ...
        {
            ModuleDefMD moduleDefMD = module;
            MetadataReader metadataReader = new MetadataReader();
            FileStream fileStream = null;
            BinaryReader binaryReader = null;
            //       fileStream = new FileStream(Console.ReadLine(), FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            byte[] tester = moduleDefMD.Metadata.PEImage.CreateReader().ReadBytes((int)moduleDefMD.Metadata.PEImage.CreateReader().Length);
            ...
            if (metadataReader.Intialize(binaryReader))
            {
                ...
                for (int i = 0; i < metadataReader.TableLengths[6]; i++)
                {
                    ...
                    if (array3[i] > 0)
                    {
                        ...
                        byte b = (byte)(binaryReader.ReadByte() & 0xff);
                        if ((b & 3) == 2)
                        {
                            num2 = b >> 2;
                            array3[i]++;
