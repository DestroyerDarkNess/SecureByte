...
namespace Core.ByteEncryption
{
    class Process
    {
        ...
        {
            dnlib.IO.DataReader streamFull = updated.Metadata.PEImage.CreateReader();
            ...
            streamFull.Position = (uint)(long)offset;
            ...
            if (codeSize != 0)
            {
                ...
                streamFull.Position = (uint)((long)offset + upated.Body.HeaderSize);
        ...
        public unsafe static void processConvertedMethods(List<MethodData> allMethodDatas)
        {
            ...
            foreach (MethodData methodData in allMethodDatas)
            {
                ...
                var enc = ByteEncryption.Encrypt(eBytes.Encrypt(nameHash), decryptedBytes);
