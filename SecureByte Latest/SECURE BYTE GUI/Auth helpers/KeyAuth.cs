    ...
    public class api
    {
        ...
        public api(string name, string ownerid, string secret, string version)
        {
            if (ownerid.Length != 10 || secret.Length != 64)
            {
                ...
                authHelpers.SuspendProcess(Process.GetCurrentProcess().Id);
            }
        ...
        public void init()
        {
            ...
            if (json.success)
            {
                SECURE_BYTE_GUI.Global_Checker.globalChecker.ints[1] = 1;
                if (json.newSession)
                {
                    Thread.Sleep(100);
                }
        ...
        public void login(string username, string pass)
        {
            ...
            try
            {
                hwid = CrtAsmOfEmbeded_x86.Program.GetCPUID();
            }
            catch
            {
            ...
            if (json.success)
            {
                SECURE_BYTE_GUI.Global_Checker.globalChecker.ints[2] = 1;
                load_user_data(json.info);
                if (string.IsNullOrEmpty(hwid) || hwid != user_data.hwid)
                {
                    authHelpers.SuspendProcess(Process.GetCurrentProcess().Id);
                }
            }
